...
class GPT3Node(BaseNode):
    ...
    def run(self, data: PipelineData, **kwargs) -> PipelineData:
        """
        运行GPT-3查询
        输入: data.content 应该是构建好的提示文本
        输出: PipelineData 包含AI回答
        """
        prompt = data.content
            ...
            try:
                ...
                if "choices" in response_data and len(response_data["choices"]) > 0:
                    ...
                    result = PipelineData(
                        ...
                        metadata={
                            'original_query': data.get_metadata('original_query', ''),
                            'prompt': prompt,
                            'model': self.model,
                            'finish_reason': response_data["choices"][0].get("finish_reason"),
                            'node_type': 'llm_response',
                            'context_used': data.get_metadata('context_used', False),
                            'num_contexts': data.get_metadata('num_contexts', 0)
                        },
                        source='GPT3Node'
                    )
