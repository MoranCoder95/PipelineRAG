...
class BaseComponent:
    ...
    def save_output(self, output: PipelineData):
        """保存节点输出"""
        if self.storage:
            try:
                self.storage.save(output.to_dict())
            except Exception as e:
                logger.error(f"存储输出失败: {str(e)}")
    ...
    def run(self, data: PipelineData, **kwargs) -> PipelineData:
        ...
        Returns:
            PipelineData: 输出的Pipeline数据
        """
        pass

    def _dispatch_run(self, **kwargs) -> PipelineData:
        """分发运行请求"""
        return self._dispatch_run_general(self.run, **kwargs)
    ...
    def _dispatch_run_general(self, run_method: Callable, **kwargs) -> PipelineData:
        """通用运行分发逻辑
        ...
        # 创建或获取输入数据
        if 'data' not in run_inputs and 'query' in kwargs:
            # 如果没有data但有query，创建PipelineData
            run_inputs['data'] = PipelineData(
                content=kwargs['query'],
                metadata={'node_type': 'query'},
                source='Query'
            )
        elif 'data' not in run_inputs and 'file_paths' in kwargs:
            # 如果没有data但有file_paths，创建PipelineData
            run_inputs['data'] = PipelineData(
                content=kwargs['file_paths'],
                metadata={'node_type': 'file_paths'},
                source='File'
            )

        # 运行方法
        output = run_method(**run_inputs, **run_params)

        # 添加额外的元数据
        output.add_metadata('node_name', self.name)
        output.add_metadata('params', params)
